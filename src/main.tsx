...
import { Activity, ArrowUpRight, Brain, CheckCircle2, Command, Cpu, Database, DownloadCloud, Gauge, KeyRound, Loader2, Lock, MessageSquareText, PlugZap, Send, Shield, Sparkles, TerminalSquare, Wifi, WifiOff, Network as NetworkIcon } from 'lucide-react';
import './styles/app.css';

type Screen = 'dashboard' | 'chat' | 'integrations' | 'network' | 'knowledge' | 'diagnostics' | 'settings';
type Role = 'noa' | 'john';
...
function App() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'noa', text: 'Hey John - Noah is online. Alpha 1.2 is online. I’ll run a startup health check automatically, then use Workspace Intelligence with properly formatted chat responses.', source: 'local' }
  ]);
      ...
      <section className="workspace">
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            <div className="status-pill muted"><Lock size={16} />Alpha 1.2</div>
          </div>
        </header>
...
function screenTitle(screen: Screen) { return ({ dashboard: 'Command Centre', chat: 'Conversation', integrations: 'Integrations', network: 'Network Core', knowledge: 'Knowledge Graph', diagnostics: 'Diagnostics', settings: 'Settings' } as const)[screen]; }
